    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span v-if="scope.row.checkTag =='S001-0'">等待</span>
          <span v-else-if="scope.row.checkTag == 'S001-1'">完成</span>
          <span v-else="scope.row.checkTag == 'S001-2'">等待</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="checkTag"
        ...
        <template slot-scope="scope">
          <span v-if="scope.row.checkTag =='S001-0'">等待</span>
          <span v-else-if="scope.row.checkTag == 'S001-1'">通过</span>
          <span v-else="scope.row.checkTag == 'S001-2'">未通过</span>
        </template>
      </el-table-column>
      <el-table-column
        label="装配树">
  ...
  export default {
    ...
    methods: {
      getdata() {   //获取数据
        ...
        //产品档案
        this.$axios.post("/dModule/queryallDModule", params).then(function (response) {
