<template>
  ...
  <h1>入库</h1>
    ...
    <el-table :data="tableData" stripe border style="width: 100%">
      <el-table-column prop="gatherId" label="入库单编号" width="180"></el-table-column>
    ...
    <!--  预告入库调度单-->
    <el-dialog width="80%" title="入库调度单" :visible="zdwinshow">

      <el-form  :modal="scFrom1">
        <el-row>
          <el-col :span="12">
            <div class="grid-content bg-purple">
              <strong style="margin-right: 220px">入库单编号:  {{scFrom1.gatherId}}</strong>
              <br>
              <br>
              <strong style="margin-right: 310px">入库理由:  {{scFrom1.reason}}</strong>
            </div>
          </el-col>
          <el-col :span="12">
            <div class="grid-content bg-purple">
              <br>
              <strong >入库详细理由: {{scFrom1.reasonexact}}</strong>
            </div>
          </el-col>
        </el-row>
        <br>
        ...
        <div>
          <el-table :data="scFrom" stripe border style="width: 100%">
            ...
            <template slot-scope="scope">
            <a href="#" @click.prevent='diaodu(scope.row)'>入库调度</a>
            </template>
            </el-table-column>
          </el-table>
        </div>
  ...
  export default {
    ...
    methods: {
      getdata() {   //获取数据
        ...
        axios.post("/sGather/Makestockinorder.action", params).then(function (response) {
      ...
      zhidin(row){
        ...
        axios.post("/sGather/akestockinordermbyid.action",params).then(function (response) {
