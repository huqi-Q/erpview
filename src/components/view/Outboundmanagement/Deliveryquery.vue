    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
                    <span v-if="scope.row.payTag =='K20'" style="color:orange">等待</span>
                    <span v-if="scope.row.payTag =='K002-1'" style="color:orange">已完成</span>
          <span v-else-if="scope.row.payTag == 'K002-2'" style="color:greenyellow"
          >已调度</span>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  export default {
    ...
    methods: {
      getdata() {   //获取数据
        var _this = this;
        this.$axios.post("/sPayDetails/querySPayDetail.action").then(function (response) {
