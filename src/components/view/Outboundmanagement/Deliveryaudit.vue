<template>
  ...
  <div>
    ...
    <el-dialog width="80%" title="出库审核" :visible="jhwinshow">
...
<script>
  export default {
    ...
    methods: {
      getdata() {   //获取数据
        var _this = this;
        this.$axios.post("/sPayDetails/querySPayDetails.action").then(function (response) {
      ...
      tjsh(){
        ...
        params.append("id", this.scgxtableData.id);
        this.$axios.post("/sPayDetails/sPayDetailsAuditsuccessful.action",params).then(function (response) {
